...
import Animated, { Extrapolation, interpolate, runOnJS, runOnUI, useAnimatedStyle, useSharedValue, withSpring, withTiming, } from 'react-native-reanimated';
...
const Overlay = ({ item, hide, image, children }) => {
    ...
    const screenHeightMinusInset = screenHeight - (overlayUnderNotch ? 0 : insets.top);
    ...
    const resetOverlay = () => {
        ...
        overlayScale.value = withTiming((item.origin?.width || 0) / screenWidth, SPRING_CONFIG);
    ...
    const onOpen = () => {
        ...
        overlayY.value = withTiming(-(item.origin?.y || 0) + (overlayUnderNotch ? 0 : insets.top), SPRING_CONFIG);
    ...
    const overlayAnimatedStyle = useAnimatedStyle(() => {
        return {
            ...
            borderRadius: interpolate(overlayProgress.value, [0, 1], [BORDER_RADIUS_TILE, BORDER_RADIUS_OVERLAY]),
    ...
    const panGesture = useMemo(() => Gesture.Pan()
        ...
        panScale.value = interpolate(event.translationY, [0, 500], [1, 0.75], Extrapolation.CLAMP);
    })
        .onEnd((event) => {
